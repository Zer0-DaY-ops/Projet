...
// ==========================================
// 1. CONFIGURATION EMAIL (À remplir)
// ==========================================
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: "TON_EMAIL@gmail.com",
    pass: "TON_MOT_DE_PASSE_APPLICATION",
  },
});
...
function isValidProduct(title, price, query) {
  ...
  if (isConsolesSearch(query)) {
    ...
    if (price < 80) return false; // Une console à moins de 80€ n'existe pas en neuf
  }
  return true;
}
...
app.post("/api/send-email", async (req, res) => {
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: `Top 10 offres pour : ${query}`,
      html:
        `<h3>Voici les meilleures offres trouvées :</h3>` +
        top
          .map(
            (r) =>
              `<p><b>${r.site}</b>: ${r.price} - <a href="${r.link}">Lien</a></p>`,
          )
          .join(""),
    });
